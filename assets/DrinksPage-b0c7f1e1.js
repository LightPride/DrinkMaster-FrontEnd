...
`,q=({placeholder:e,options:n,onChange:o})=>{const r=n.map(t=>({value:t,label:t}));return f.jsx(Ue,{placeholder:e,options:r,onChange:o,classNamePrefix:"react-select",isClearable:!0,isSearchable:!0})},Ae=()=>{const e=V(),n=I(oe),o=I(ie),[r,t]=O.useState(""),[c,i]=O.useState(""),[a,s]=O.useState("");O.useEffect(()=>{e(J()),e(Y());const l=localStorage.getItem("searchQuery");l&&(t(l),e(C({name:l})));const p=localStorage.getItem("selectedCategory");p&&i(p);const j=localStorage.getItem("selectedIngredient");j&&s(j)},[e]);const d=l=>{const p=l.target.value;t(p),localStorage.setItem("searchQuery",p)},y=l=>{if(l){const p=l.value;i(p),localStorage.setItem("selectedCategory",p)}else i(""),localStorage.removeItem("selectedCategory")},b=l=>{if(l){const p=l.value;s(p),localStorage.setItem("selectedIngredient",p)}else s(""),localStorage.removeItem("selectedIngredient")};O.useEffect(()=>{e(C({name:r,category:c,ingredient:a})),localStorage.setItem("searchQuery",r)},[e,r,c,a]);const w=n.map(l=>l.category).filter(l=>l!==c),m=o.map(l=>l.title).filter(l=>l!==a);return f.jsxs(ze,{children:[f.jsx(Me,{id:"filterInput",type:"text",placeholder:"Enter the text",value:r,onChange:d,debounceTimeout:1e3}),f.jsx(q,{id:"categorySelect",placeholder:"All categories",options:w,onChange:y}),f.jsx(q,{id:"ingredientSelect",placeholder:"Ingredients",options:m,onChange:b})]})},Qe=Ae,qe=h.li`
  width: 335px;
...
`,Xe=({drinkThumb:e,drinkTitle:n,id:o})=>f.jsxs(qe,{children:[f.jsx(Ve,{src:e,alt:n,onError:r=>{r.currentTarget.src=ae}}),f.jsxs(He,{children:[f.jsx(Fe,{children:n}),f.jsx(Ge,{to:`/drinks/${o}`,children:"See more"})]})]}),Je=h.ul`
  display: flex;
  flex-direction: row;
  flex-wrap: wrap;
  gap: 28px;
